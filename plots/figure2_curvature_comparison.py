...
ps = np.load(os.path.join(RESULTS, 'euclid_ps.npy'))

data = {
    r'$q=6$  (flat, Euclidean control)':   (os.path.join(RESULTS, 'euclid_b1.npy'), 625,  '#2b6cb0'),
    r'$q=7$  ($\{3,7\}$)':                 (os.path.join(RESULTS, 'hyp37_b1.npy'),  617,  '#c05621'),
    r'$q=8$  (more negatively curved)':    (os.path.join(RESULTS, 'hyp38_b1.npy'),  609,  '#9b2c2c'),
}
...
fig.tight_layout()
fig.savefig(os.path.join(FIGURES, 'curvature_persistence_comparison.png'), dpi=160)
print('saved')
